...
export default function Rules({ rules, remove, toggle }) {
  return (
    <Box>
      ...
      <div className="text-sm tracking-tight text-prim/50 mb-6">
        Define rules to bind gamepad button to Midi messages.
      </div>
      <div className="border border-prim/10 rounded-sm overflow-x-scroll">
          ...
          <tbody>
            {rules
              ...
              .map((r) => (
                <tr
                  key={r.id}
                  className="*:h-12 *:px-4 *:align-middle hover:bg-prim/5 border-t border-prim/10"
                >
                  <td>
                    G<span className="font-bold">{r.gamepadIndex}</span>
                  </td>
                  <td>
                  ...
                  <td>
                    ...
                    <span className="font-bold ml-1">
                      {r.midiMessageChannel + 1}
                    </span>
                  </td>
                  <td>{r.midiMessageValue1}</td>
                  <td>{r.midiMessageValue2 ?? "%"}</td>
                  <td>
                    <button
                      type="button"
                      onClick={() => remove(r.id)}
